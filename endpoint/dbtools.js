...
const dbtools    = express.Router({})
const dateFormat = require('dateformat');

//--------------------------------------------------
// crawling  functions
//--------------------------------------------------
dbtools.get('/getYoutube', async (req, res)=>{
  try{
...
// test functions
//--------------------------------------------------
dbtools.post('/', async (req, res)=>{
  try{
    log('test req.body= :', req.body)
    res.json({msg:RCODE.OPERATION_SUCCEED, data:{item:' DB tools Good Server~~~'}})
  }
  catch(err){
    // TODO: test auto save 
    log('err=',err)
...
dbtools.get('/newestUser', async (req, res)=>{
  try{
...
dbtools.get('/oldestUser', async (req, res)=>{
  try{
...
dbtools.get('/randomUser', async (req, res)=>{
  try{
...
dbtools.get('/newestPurchase', async (req, res)=>{
  try{
...
dbtools.get('/oldestPurchase', async (req, res)=>{
  try{
...
dbtools.get('/excelTest', async (req, res)=>{
  try{
...
dbtools.get('/timestamp', async (req, res)=>{
  try{
    let qry = {}
    let voices = []
    let total = 0
